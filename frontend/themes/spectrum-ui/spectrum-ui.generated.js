...
window.Vaadin.Flow['_vaadintheme_spectrum-ui_globalCss'] = window.Vaadin.Flow['_vaadintheme_spectrum-ui_globalCss'] || [];
export const applyTheme = (target) => {
  
  const injectGlobal = (window.Vaadin.Flow['_vaadintheme_spectrum-ui_globalCss'].length === 0) || (!window.Vaadin.Flow['_vaadintheme_spectrum-ui_globalCss'].includes(target) && target !== document);
  if (injectGlobal) {
    injectGlobalCss(stylesCss.toString(), target);
    
    window.Vaadin.Flow['_vaadintheme_spectrum-ui_globalCss'].push(target);
  }
  if (!document['_vaadintheme_spectrum-ui_componentCss']) {
    
    document['_vaadintheme_spectrum-ui_componentCss'] = true;
  }
